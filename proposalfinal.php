...
<p>Any supplier interested in any tender may use this form to submit the proposal..The proposal will be judge by UiTM officer. You have to check the result at least three day after the due date of the tender.</p>
    ...
    <div class="col-75">
      <?php
      $sql="SELECT tender_id FROM tender";
      $tender_id = mysqli_query($con,$sql);
      $tenderlist=mysqli_fetch_all($tender_id);
      ?>

      <select name="tender">
      <?php
      foreach($tenderlist as $currTender){
        $tender_id=$currTender[0];
        echo"<option value='$tender_id'>$tender_id</option>";
      }
      ?>
      </select>
    </div>
